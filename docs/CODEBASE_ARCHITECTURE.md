...
Community features are separated logically and can use a separate database through `COMMUNITY_DATABASE_URL`. If no community DB is configured, the app currently creates the community pool as a clone of the primary pool and runs migrations from `database/community` against that primary database.

### 13.1 User-facing Page Routes

| Path | Handler | Purpose |
|---|---|---|
| `/community` | `page_community` | Main SPA shell with HTMX-driven tabs (Feed / Announcements / Circle / Challenges / AMAs / Leaderboard / Saved / Search / Notifications / Messages). |
| `/community/post/:id` | `page_community_post` | SSR post detail page with OG meta and comments. |
| `/community/hashtag/:tag` | `page_community_hashtag` | Paginated hashtag landing page (`?page=N`). |
| `/community/me` | `page_community_my_profile` | Own profile, redirects to login when anonymous, shares `community-profile.html` with `is_own=true`. |
| `/community/u/:user_id` | `page_community_user_profile` | Public profile page; renders the same template with `is_own=false` for non-owners. Records a row in `profile_views`. |
| `/community/badge/:id` | `page_community_badge` | Badge detail page. |
| `/community/partials/feed/list` | `community_feed_list_htmx` | HTMX partial used by feed + saved tab + infinite-scroll sentinel. Supports `?source=bookmarks`, `?feed_mode`, `?sort_by`, `?page`. |
| `/community/partials/announcements/list` | `community_announcements_list_htmx` | HTMX partial for announcements with category filter. |
| `/community/partials/:tab` | `community_htmx_partial` | Catch-all for the tab partials (feed / announcements / circle / challenges / ama). |

### 13.2 API Surface

Grouped by feature area; every write endpoint is CSRF-protected and ban-checks the caller. Notable routes added in the 2026-Q2 community workstream are flagged with †.

**Posts** — `POST /api/community/posts`, `GET|PUT|DELETE /api/community/posts/:id`, `POST /api/community/posts/:id/report`, `POST /api/community/posts/:id/reactions`, `POST|DELETE /api/community/posts/:id/bookmark`, `GET /api/community/posts/:id/bookmark/status`, `POST /api/community/posts/:id/poll/vote`, `GET /api/community/posts/:id/poll`.

**Comments** — `GET|POST /api/community/posts/:id/comments`, `PUT|DELETE /api/community/comments/:id`, `POST /api/community/comments/:id/reactions`. Nested replies (depth 1) via `parent_comment_id` on the create payload.

**Profile** — `GET|PUT /api/community/profile`, `GET /api/community/profile/me`, `GET /api/community/profile/:id`. Followers + following lists at `/api/community/profile/:id/{followers,following}?page=N` (paginated, has_more flag). †`/posts`, †`/comments`, †`/media`, †`/activity` per-user feeds. †`GET /api/community/profile/me/analytics` (30-day rollup), †`GET /api/community/profile/me/moderation-log`, `POST /api/community/profile/verify-request`.

**Follows + blocks + mutes** — `POST|DELETE /api/community/follow/:id`, `POST|DELETE /api/community/users/:id/block`, `POST|DELETE /api/community/users/:id/mute`, `GET /api/community/blocks`, `GET /api/community/mutes`.

**Circles** — `POST /api/community/circles`, `GET /api/community/circles/me`, `GET /api/community/circles/leaderboard`, `PUT|DELETE /api/community/circles/:id`, `POST /api/community/circles/:id/privacy`, `POST|DELETE /api/community/circles/:id/request`, join/leave/invite handlers.

**AMAs** — `GET /api/community/amas`, `GET /api/community/amas/:id`, `POST /api/community/amas/:id/questions`, `POST /api/community/amas/:id/questions/:qid/upvote`.

**Challenges** — `GET /api/community/challenges` (auto-tracked via `challenges.rs::increment_progress` on relevant actions).

**Leaderboard** — `GET /api/community/leaderboard?limit=N&period=alltime|month|week` (global), `GET /api/community/circles/leaderboard` (per-circle).

**Notifications** — `GET /api/community/notifications`, `POST /api/community/notifications/read-all`, `POST /api/community/notifications/:id/read`, `GET|PUT /api/community/notifications/preferences` (per-type opt-out JSONB).

**Hashtags + mentions + search** — `GET /api/community/hashtags/trending`, `GET /api/community/hashtags/:tag`, †`GET /api/community/hashtags/suggest?q=`, †`GET /api/community/mentions/suggest?q=`, `GET /api/community/search?q=&type=`.

**Verification + appeals** — `POST /api/community/profile/verify-request`, `GET /api/admin/community/verification-requests`, `POST /api/admin/community/verification-requests/:id/review`. `POST /api/community/appeals`, `GET /api/admin/community/appeals`, `POST /api/admin/community/appeals/:id/review`.

**Admin** — `POST /api/admin/community/users/:id/{ban,mute,warn,shadowban,mod-notes,xp}`, `GET /api/admin/community/audit-log` (filterable by `target_user_id`), `GET /api/admin/community/{posts,comments,reports,users,amas,challenges,announcements,circles,leaderboard,stats}`, plus per-resource action handlers.

### 13.3 Migration Ledger (`database/community/`)

| File | Purpose |
|---|---|
| 001-007 | Posts, comments, reactions, announcement categories, community_profiles, indexes, social layer, content reports. |
| 008 | Circles table + `xp_total / level / level_name` on community_profiles. |
| 009-012 | AMAs, reviews, challenges, notifications. |
| 013-018 | Moderation, shadowban, link previews, ban appeals, circle requests, audit log. |
| 019-021 | Bookmarks, polls, hashtags. |
| 025-026 | Token-gated circles, XP non-negative constraint. |
| 027-029 | Block/mute, comment edits (`original_content`, `edited_at`), comment reactions. |
| 030-031 | Notification prefs (twice — see drift note). |
| 031-032 | Verified-owner requests, comment threading (`parent_comment_id`), verification_requests, DM tables. |
| 033 | †Nested comment replies (FK). |
| 034 | †`community_profiles.is_verified_owner` (denormalized for inheritance on new posts). |
| 035 | †`profile_views` table for analytics. |
| 036 | †Composite `(user_id, created_at DESC)` indexes on posts + comments. |

Drift to watch: several numbers (031 / 032 / 033 / 034) ship two files each, applied alphabetically. Tracked in `qa-reports/community-migration-cleanliness.md`.

### 13.4 Frontend JS Modules

`frontend/platform/static/js/`:

- `community-feed.js` — feed, composer, comments + replies, follow, profile modal, hashtag filter, lightbox, drag-drop, Ctrl+Enter, mention/handle resolution. Wrapped in `window.initCommunityFeed`.
- `community-circles.js` — My Circle tab + circle modals + leaderboard.
- `community-amas.js` — AMA tab.
- `community-challenges.js` — challenge cards with action deeplinks.
- `community-announcements.js` — no-op shim (announcements link directly to SSR post detail).
- `community-autocomplete.js` — composer mention/hashtag autocomplete.
- `community-ban-appeal.js` — ban banner + appeal modal.
- `community-mobile-tabs.js` — horizontal-scroll + swipe gesture for the topbar tabs on mobile.
- `community-profile.js` — `/community/me|u/:id` page; lazy-loads 9 tabs with pagination.
- `admin-community-*.js` — appeals, circles, comments, index, leaderboard, reports, users, verified-owner.

### 13.5 Frontend CSS

- `community.css` — primary stylesheet (~3500 lines). All `ds-*` and `community-*` primitives.
- `community-profile.css` — the `/community/me|u/:id` page.
- `leaderboard.css` — shared `lb-*` topbar/page-shell primitives. **Loaded after `community.css`** so its `.lb-topbar-tabs { display: none }` at `<=768px` requires the more-specific `.lb-topbar-tabs.community-topbar-tabs` override (already wired). Anyone adding a third community CSS file that needs to override leaderboard must keep this in mind.

### 13.6 Known Quirks

- **MiniJinja has no `tojson` filter.** Don't reach for `{{ x | tojson }}` in any community template — render as a regular string and let JS deserialize, or expose the value via a `data-*` attribute.
- **Templates do not auto-reload.** `templates::create_engine()` loads from `frontend/platform` at startup. Restart the backend preview after every template edit.
- **Modals use `style="display:none"` as a state toggle**, not a visual rule. JS flips `style.display = 'flex'` to open. This is intentional and exempt from the WS2 inline-style purge.
- **Mobile topbar override.** See 13.5 above.
- **Pre-existing legacy `/profile` route** in `auth/routes.rs:page_profile` is unrelated to community; do not merge with `/community/u/:id`.
