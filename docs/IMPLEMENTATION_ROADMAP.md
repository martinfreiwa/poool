...
| **14.8.13** | Badge detail + earning rules | `GET /api/community/badges/:id`; `/community/badge/:id` page; link from profile modal | `✅ DONE` | `Claude Opus 4.7` | `❌` | Backend endpoint + SSR page `community-badge.html` + `.community-badge-detail/*` CSS family. Smoke-tested. Profile-modal link is a small follow-up. |
...
| **14.8.15** | Notification preferences page | `GET/PUT /api/community/notification-prefs`; checkbox grid sub-page `/settings/notifications/community` | `✅ DONE (backend)` | `Claude Opus 4.7` | `✅` | Migration `030_notification_prefs.sql` adds 8 per-channel booleans on `community_profiles`. GET/PUT endpoints with partial-update via COALESCE. Smoke-tested. Settings sub-page UI is a follow-up. |
| **14.8.16** | Verified-owner request flow | Migration `verified_owner_requests`; user submit + admin review endpoints | `✅ DONE (backend)` | `Claude Opus 4.7` | `✅` | Migration `031_verified_owner_requests.sql` + 4 routes (user submit/list, admin list/patch with `community.manage` gate). Smoke-tested. UI surfaces are FE follow-ups. |
...
| **14.8.19** | Search filters | `?date_from=`, `?date_to=`, `?author_id=`, `?min_engagement=`; filter chips on Search tab | `✅ DONE (backend)` | `Claude Opus 4.7` | `✅` | `SearchQuery` extended with 4 new fields; server parses dates leniently; SQL filters on created_at range + author_id + engagement sum. Smoke-tested. FE filter chips are a follow-up. |
| **14.8.20** | Direct messages (DMs) | New `dm_threads` + `dm_messages` tables; thread + message endpoints; split-pane UI with new `data-tab="community-dms-tab"` | `✅ DONE (backend)` | `Claude Opus 4.7` | `✅` | Migration `034_dms.sql` (normalised pair via CHECK + UNIQUE). 4 routes — list threads, create thread, list messages, post message. Block enforcement via `dm_block_exists` helper. Smoke-tested (empty threads list returns). DM tab UI is a follow-up. |
