...
| ID | Task | Description | Status | Assignee | Tested? | Notes |
|:---|:---|:---|:---|:---|:---|:---|
| **14.8.1** | Ban appeal submission flow (user UI) | Banner + modal on `/community` when `is_community_banned=true`; POST to existing `submit_ban_appeal` | `❌ NOT STARTED` | - | `❌` | P0 safety/compliance. Backend exists at `routes.rs:4844`. |
| **14.8.2** | Block / mute another user (self-service) | New tables + endpoints + profile-modal actions + `/community/blocks` settings sub-page | `❌ NOT STARTED` | - | `❌` | P0 user safety. Migration: `block_relationships`, `mute_relationships`. |
| **14.8.3** | Post edit (frontend wire-up) | Kebab menu on own post; opens composer modal prefilled; PUT to existing endpoint | `❌ NOT STARTED` | - | `❌` | Backend exists at `routes.rs:924`. |
| **14.8.4** | Post delete (own) — frontend wire-up | Confirm dialog + DELETE to existing endpoint | `❌ NOT STARTED` | - | `❌` | Backend exists at `routes.rs:956`. |
