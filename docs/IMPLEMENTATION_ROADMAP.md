...
|:---|:---|:---|:---|:---|:---|
| `2026-05-11 03:00` | `Claude Opus 4.7` | `14.8 Phase 2 handoff brief` | `docs/community/COMMUNITY_GAP_CLOSEOUT_PHASE2.md, docs/IMPLEMENTATION_ROADMAP.md` | `✅ Check-Out` | Authored `docs/community/COMMUNITY_GAP_CLOSEOUT_PHASE2.md` for the next agent — companion brief covering the 12 remaining 14.8.X features (post the 12 that landed today). Includes grep-verified pre-flight notes, a recommended commit order starting at 14.8.17 (Poll viz polish), and the existing-fixture inventory for `tests/e2e/test_community.py`. Pickup link added to the Phase 14.8 section header. |
| `2026-05-11 02:45` | `Claude Opus 4.7` | `14.8.6 Comment reactions` | `backend/src/community/{routes.rs,models.rs}, database/community/029_comment_reactions.sql, frontend/platform/static/js/community-feed.js, frontend/platform/static/css/community.css, tests/e2e/test_community.py` | `✅ Check-Out` | Commit `7e88df2`. New `comment_reactions` table + trigger; comments.reaction_count column. POST /api/community/comments/:id/reactions toggles per (comment, user, type). 🔥 reaction pill on every comment row. 1 new passing e2e test. |
| `2026-05-11 02:30` | `Claude Opus 4.7` | `14.8.5 Comment edit (own)` | `backend/src/community/{routes.rs,models.rs}, database/community/028_comment_edits.sql, frontend/platform/static/js/community-feed.js, frontend/platform/static/css/community.css, tests/e2e/test_community.py` | `✅ Check-Out` | Commit `eedffff`. New `PUT /api/community/comments/:id` + migration 028. Inline edit affordance on own-comment rows; "Edited" suffix on timestamp. 1 new passing e2e test. |
...
> **Pickup brief for the next agent:** `docs/community/COMMUNITY_GAP_CLOSEOUT_PHASE2.md` — companion brief listing the 12 remaining features with a verified recommended commit order. Start at 14.8.17 (Poll viz polish, no migration).
...
| **14.8.5** | Comment edit | Add `PUT /api/community/comments/:id`; migration adds `edited_at` + `original_content`; inline edit on `.community-comment-row` | `✅ DONE` | `Claude Opus 4.7` | `✅` | Commit `eedffff`. Migration `028_comment_edits.sql`. PUT route reuses create_comment guards (ban + automod + locked-thread + sanitisation). FE: edit button on own comment rows with inline textarea + Save/Cancel; "Edited" suffix on timestamp. 1 new passing e2e test. |
| **14.8.6** | Comment reactions | Extend `reactions` polymorphic or new `comment_reactions` table; single `fire` button per comment | `✅ DONE` | `Claude Opus 4.7` | `✅` | Commit `7e88df2`. Decision: new `comment_reactions` table (mirrors `reactions` structure, lower-risk than polymorphic refactor). Migration `029_comment_reactions.sql` adds table + trigger + `comments.reaction_count`. Frontend renders 🔥 pill on every comment row. 1 new passing e2e test. |
| **14.8.7** | Profile picture upload | `/api/upload/avatar` writes `community_profiles.avatar_url`; affordance in `edit-profile-modal` | `✅ DONE` | prior phase | `✅` | Already shipped. `POST /api/upload/avatar` in `storage/routes.rs:38` (GCS with local fallback). Writes to `users.avatar_url` — community pages bridge via `user_bridge`, no `community_profiles.avatar_url` column needed. JS handler `uploadProfileAvatar` in `community-feed.js:2205` paints preview + flushes sidebar. |
| **14.8.8** | Hashtag browse page | SSR `GET /community/hashtag/:tag` + HTMX partial; reuses `get_posts_by_hashtag` + post-list partial | `✅ DONE` | prior phase | `✅` | Already shipped. `GET /community/hashtag/:tag` at `main.rs:1006` → `page_community_hashtag` (main.rs:2864) renders `community-hashtag.html` with `.community-hashtag-banner` and reuses `get_hashtag_feed_data`. |
| **14.8.9** | Global leaderboard view | New `/api/community/leaderboard/global`; partial; new tab or My Circle sub-section | `✅ DONE` | parallel agent / audit task 2 | `✅` | Commit `e75ec41` (MVP) + audit-task-2 follow-up. Backend `get_global_leaderboard` at routes.rs:4321; tab panel + Alpine controller in community.html. Path is `/api/community/leaderboard` (brief's `/leaderboard/global` was incorrect). Audit task 2 added `?period=week|month|alltime` via `LeaderboardPeriod` enum + windowed `xp_ledger` aggregation in `community::xp::get_user_leaderboard_for_period`, and now respects `leaderboard_preferences.visible=false` (anonymizes hidden users). |
| **14.8.10** | Announcement detail page | SSR `GET /community/announcement/:id`; reuse post-card partial | `✅ DONE` | parallel agent | `❌` | Commit `0409ab4`. Announcements link to SSR post detail. |
| **14.8.11** | Challenge participation flow | Join / submit / vote endpoints + expand `community-challenges.js` from 74 lines | `✅ DONE (passive model)` | parallel agent | `⏸` | Commit `9181912`. Current model is passive — XP accrues via existing actions, deeplinks per challenge. Vote-based submission challenges need `challenge_submissions` table; out of 14.8.11 scope. |
| **14.8.12** | Nested comment replies | Migration `parent_comment_id` self-FK; thread depth cap of 2 | `✅ DONE (backend)` | `Claude Opus 4.7` | `❌` | Migration `032_comment_threading.sql` + `service::create_comment(parent_id)` depth cap of 2 enforced server-side. `CreateCommentReq.parent_comment_id`. JSON responses return `parent_comment_id` + `reaction_count`. Frontend reply UI is a follow-up. |
| **14.8.13** | Badge detail + earning rules | `GET /api/community/badges/:id`; `/community/badge/:id` page; link from profile modal | `✅ DONE` | `Claude Opus 4.7` | `❌` | Commit `f9b1030` follow-up. Backend endpoint + SSR page `community-badge.html` + `.community-badge-detail/*` CSS family. Profile-modal badges now link to `/community/badge/:id` (BadgeDisplay carries id). |
| **14.8.14** | Asset reviews surface inside community | Backend CRUD exists at `routes.rs:2309`; cross-link from asset detail + Feed filter pill | `⏸ DEFERRED` | - | `❌` | Backend already complete; FE integration on property/commodity pages is a separate slice for the asset-detail page owner. |
| **14.8.15** | Notification preferences page | `GET/PUT /api/community/notification-prefs`; checkbox grid sub-page `/settings/notifications/community` | `✅ DONE` | `Claude Opus 4.7` | `✅` | Commit `f9b1030`. Migration `030_notification_prefs.sql` adds 8 per-channel booleans. New `/settings/notifications/community` sub-page with 8 toggle rows. Each toggle PUTs only the changed key (partial-update via COALESCE so siblings aren't clobbered). |
| **14.8.16** | Verified-owner request flow | Migration `verified_owner_requests`; user submit + admin review endpoints | `✅ DONE` | `Claude Opus 4.7` | `✅` | Commit `f9b1030`. Migration `031_verified_owner_requests.sql` + 4 routes. User-facing submit form sits in edit-profile-modal. New `admin/community/verified-owner-requests.html` with status filter + approve/reject modal (notes optional). PATCH route already awards the badge + flips profile/posts flags server-side. |
| **14.8.17** | Poll result viz polish | Bar-per-option using `.ds-progress`; mark user's selection | `✅ DONE` | prior to 14.8 phase | `✅` | Already in `community-feed.js:1685` — `.poll-option-bar` proportional fills, voted-state marker, expired-state. No further work needed. |
| **14.8.18** | Mentions/hashtag autocomplete | `/api/community/{users,hashtags}/autocomplete?q=`; floating list in composer; 150ms debounce | `✅ DONE` | parallel agent | `✅` | Commit `f6c7c94`. Endpoints `/api/community/mentions/suggest` + `/hashtags/suggest` (renamed from brief's `/autocomplete`); FE module `community-autocomplete.js`. |
| **14.8.19** | Search filters | `?date_from=`, `?date_to=`, `?author_id=`, `?min_engagement=`; filter chips on Search tab | `✅ DONE` | `Claude Opus 4.7` | `✅` | Commit `f9b1030`. Backend `SearchQuery` extended with 4 fields; SQL filters on created_at range + author_id + engagement sum. Search tab in `community.html` now has date-from / date-to / min-engagement inputs + removable chips that re-query on change. |
| **14.8.20** | Direct messages (DMs) | New `dm_threads` + `dm_messages` tables; thread + message endpoints; split-pane UI with new `data-tab="community-dms-tab"` | `✅ DONE` | `Claude Opus 4.7` | `✅` | Commit `f9b1030`. Migration `034_dms.sql` (normalised pair). 4 routes — list threads, create thread, list messages, post message. Block enforcement via `dm_block_exists` helper. New `Messages` tab with split-pane UI (thread list + active conversation), 8-second message poll while open, user-search modal for new conversations. |
| **14.8.21** | Admin: Ban appeals review UI | New `admin/community/appeals.html`; table + approve/deny modal | `✅ DONE` | parallel agent | `✅` | Commit `b5c3568`. |
| **14.8.22** | Admin: User audit log viewer | New panel in `admin/community/user-detail.html` using `ds-table` | `✅ DONE` | parallel agent | `✅` | Commit `e4c31a0`. |
| **14.8.23** | Admin: AMA status/answer/feature wiring | Wire existing endpoints into `admin/community/amas.html` | `✅ DONE` | parallel agent | `✅` | `admin/community/amas.html` is 753 lines with `/api/admin/community/amas/:id/status`, `/answer`, `/feature` wired. |
| **14.8.24** | Admin: Community settings page | Migration `community_settings`; `GET/PUT /api/admin/community/settings`; new `admin/community/settings.html` | `✅ DONE` | `Claude Opus 4.7` | `❌` | Migration `033_community_settings.sql` seeded with 5 keys. GET/PUT routes (community.view / .manage gated). New `admin/community/settings.html` + `admin-community-settings.js` with auto-typed inputs. |
...
| **19.2** | Attribution Middleware | HttpOnly cookie (30-day TTL) on `?ref=XYZ` clicks. On registration, bind `referred_by_id` to user. Fallback: manual code field. | `✅ DONE` | `Claude Opus 4.7` | `✅` | Commit `2d7807e`. `rewards::attribution::capture_referral` middleware reads `?ref=`, sanitizes to `[A-Za-z0-9_-|]` clamped to 80 chars, writes `poool_referral` cookie HttpOnly + SameSite=Lax + 30-day Max-Age. First-touch wins: existing non-empty cookie not overwritten. Signup handler already reads cookie + falls through to `attribute_affiliate_referral`. 3 unit tests pass. |
