...
(function () {
  ...
  async function loadCommissions() {
    ...
    try {
      ...
      // Update summary cards if data provided (simulated or from API)
      if (data.summary) {
        const totalEarnedEl = document.getElementById('commissions-total-earned');
        const availableEl = document.getElementById('commissions-available-balance');
        const nextPayoutEl = document.getElementById('commissions-next-payout');
        
        if (totalEarnedEl) totalEarnedEl.textContent = (data.summary.total_earned_cents / 100).toLocaleString();
        if (availableEl) availableEl.textContent = (data.summary.available_balance_cents / 100).toLocaleString();
        if (nextPayoutEl) nextPayoutEl.textContent = data.summary.next_payout_date || '--';
      }
      ...
      tbody.innerHTML = '<tr class="data-table__row"><td class="data-table__td" colspan="5" style="text-align:center; padding:32px 0; color:#F04438; border-bottom:none;">Failed to load commissions. Please try again.</td></tr>';
    }
  }
