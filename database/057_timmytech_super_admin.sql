-- Migration: Create super_admin account for timmytech@gmail.com
-- Following the same pattern as 015_jonas_admin.sql
BEGIN;
...
BEGIN
    -- Insert user (placeholder password hash — user must reset via forgot-password flow)
    INSERT INTO users (email, password_hash, email_verified, status)
    VALUES ('timmytech@gmail.com', '$2b$12$LQv3c1VqBWrtQGR9P1QGTeO9C.oTqO4Vl.oYtO4Vl.oYtO4Vl.oYt', TRUE, 'active')
    ON CONFLICT (email) DO NOTHING;

    SELECT id INTO v_user_id FROM users WHERE email = 'timmytech@gmail.com';

    -- Ensure profile exists
    INSERT INTO user_profiles (user_id, first_name, last_name)
    VALUES (v_user_id, 'Timmy', 'Tech')
