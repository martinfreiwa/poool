...
# Define a function to rewrite each card into the requested 3-column layout.
# We will match the entire insights-content horizontally and rewrite it.
# We know the specific base_ids we generated in the last script.

def rewrite_card(html, base_id):
    # Match the card's inner insights-content.
    pattern = re.compile(rf'(<div class="insights-content".*?)((?:<div class="insights-icon-wrapper">.*?</div>\s*</div>\s*</div>\s*</div>\s*</div>\s*</div>))', re.DOTALL)
    # Wait, regex for this is tricky because of nested divs.
    pass

# We can just run a python generator script over the 6 IDs exactly like last time.
SVGS = {
...
def generate_3col_card(id_attr, base_id, title, svgs_key, show_currency=True):
    svg = SVGS[svgs_key]
    style = BLUE
    ...
    # Layout:
    # <div Row>
    #   <div Col 1 (Icon) flex-shrink:0></div>
    #   <div Col 2 (Text) flex-grow:1 flex-col> <Title/> <Value/> </div>
    #   <div Col 3 (Badge) flex-shrink:0></div>
    # </div>

    return f"""<div id="{id_attr}" class="quick-insights-card">
              <div class="insights-content" style="display: flex; flex-direction: row; align-items: center; gap: 16px; width: 100%;">
                
                <!-- Column 1: Icon -->
                <div class="insights-icon-wrapper" style="flex-shrink: 0;">
                ...
                <div style="display: flex; flex-direction: column; gap: 4px; flex-grow: 1;">
                  <div class="insights-header" style="margin-bottom:0;">
                    <span class="insights-title" style="white-space:nowrap; color:#667085;">{title}</span>
                  </div>
                ...
                <!-- Column 3: Badge -->
                <div class="financials-change change-increase" style="position:static; height:24px; display:inline-flex; align-items:center; padding: 2px 8px; border-radius: 6px; border: 1px solid #e5e7eb; flex-shrink: 0;">
...
html = replace_card(html, "key-financials-card-monthly-income-gradient", "key-financials-card-total-rental-gradient", generate_3col_card("key-financials-card-monthly-income-gradient", "monthly-income", "Monthly income", "wallet"))

# Card 2: Total rental income
html = replace_card(html, "key-financials-card-total-rental-gradient", "key-financials-card-total-appreciation-gradient", generate_3col_card("key-financials-card-total-rental-gradient", "total-rental", "Total rental income", "bldg"))

# Card 3: Total appreciation
html = replace_card(html, "key-financials-card-total-appreciation-gradient", "quick-insights-card-number-of-properties", generate_3col_card("key-financials-card-total-appreciation-gradient", "total-appreciation", "Total appreciation", "lineUp"))

# Card 4: Number of properties
html = replace_card(html, "quick-insights-card-number-of-properties", "quick-insights-card-occupancy-rate", generate_3col_card("quick-insights-card-number-of-properties", "insights-value-number-of-properties", "Number of properties", "home", show_currency=False))

# Card 5: Occupancy rate
html = replace_card(html, "quick-insights-card-occupancy-rate", "quick-insights-card-annual-rental-yield", generate_3col_card("quick-insights-card-occupancy-rate", "insights-value-occupancy-rate", "Occupancy rate", "cal", show_currency=False))
...
new_html6 = generate_3col_card("quick-insights-card-annual-rental-yield", "insights-value-annual-rental-yield", "Annual rental yield", "award", show_currency=False)
