...
fn test_comment_validation_length() {
    ...
    assert!(
        validation::validate_comment_length(&too_long).is_err(),
        "Should return error for >3000 chars"
    );

    let too_short = "  "; // Trims to empty
    assert!(
        validation::validate_comment_length(too_short).is_err(),
        "Should catch effectively empty comments"
    );
}
