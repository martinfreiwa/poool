...
This crate sets up the Tokio runtime, initializes configuration, structured logging,
...
This file exposes the modules and a `run()` entrypoint so that external
integration tests (in `tests/`) can build the same router the binary uses
via `build_platform_router(state)`.
*/
...
// Re-export AppState for external crates (integration tests) to construct.
pub use auth::routes::AppState;
...
/// Library entry point used by `bin/poool-backend` (and equivalent to the
/// previous `#[tokio::main] async fn main`). Returns once the HTTP server
/// terminates or fails.
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let platform_router = build_platform_router(state);
...
/// Build the platform Router (login, dashboard, public APIs) with all
/// middleware wired up. Extracted from `run()` so integration tests can
/// exercise the same routing the production binary uses. The router is
/// already `with_state`-bound; callers don't need to (and can't) pass state
/// at request time.
